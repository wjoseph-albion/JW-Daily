...
if mode=='Editor & Settings':
    ...
    st.subheader('Money Market 7-Day Yields');money=st.data_editor(pd.read_csv(MM,dtype=str,keep_default_na=False),use_container_width=True,hide_index=True,disabled=['Fund','Ticker'],column_config={c:st.column_config.TextColumn(c) for c in ['Yield','As Of','Source','Notes']})
...
st.divider();st.subheader('Cash Alternatives');st.dataframe(pd.read_csv(MM,dtype=str,keep_default_na=False),use_container_width=True,hide_index=True)
