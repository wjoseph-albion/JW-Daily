...
def sector_chart(rows,key,title):
    ...
    lo=float(values.min()); hi=float(values.max()); span=max(hi-lo,1.0); pad=max(span*0.04,0.35)
    fig=go.Figure(go.Bar(x=values,y=d.index,orientation='h',marker_color=['#2D6A9F' if n=='S&P 500' else '#2E7D32' if v>=0 else '#C62828' for n,v in d.items()],text=[f'{v:+.1%}' for v in d],textposition='auto',insidetextanchor='end',cliponaxis=False))
    fig.update_layout(title=title,height=475,plot_bgcolor='white',margin=dict(l=105,r=55,t=55,b=40),xaxis=dict(range=[min(0,lo)-pad,max(0,hi)+pad],ticksuffix='%',zeroline=True,zerolinecolor='#AAB4BC'),yaxis=dict(automargin=True),uniformtext_minsize=10,uniformtext_mode='show')
    return fig
...
for col,z in zip(cols,s.get('risk',[])):
    with col:st.metric(z['Asset'],f"{z['Current']:.2f}%" if z['Yield'] else f"{z['Current']:,.0f}",f"{z['Change']:+.2f} pts" if z['Yield'] else f"{z['Change']:+.2%}")
