...
def sector_chart(rows,key,title):
    d=pd.DataFrame(rows).set_index('Description')[key].dropna().sort_values(); values=d*100
    lo=float(values.min()); hi=float(values.max()); span=max(hi-lo,1.0); pad=max(span*0.18,1.25)
    fig=go.Figure(go.Bar(x=values,y=d.index,orientation='h',marker_color=['#2D6A9F' if n=='S&P 500' else '#2E7D32' if v>=0 else '#C62828' for n,v in d.items()],text=[f'{v:+.1%}' for v in d],textposition='outside',cliponaxis=False))
    fig.update_layout(title=title,height=475,plot_bgcolor='white',margin=dict(l=100,r=125,t=55,b=40),xaxis=dict(range=[min(0,lo)-pad,max(0,hi)+pad],ticksuffix='%',zeroline=True,zerolinecolor='#AAB4BC'),yaxis=dict(automargin=True),uniformtext_minsize=10,uniformtext_mode='show')
    return fig
...
if mode=='Editor & Settings':
    ...
    st.subheader('Economic Indicators - Manual Input');econ=st.data_editor(pd.read_csv(ECON),use_container_width=True,hide_index=True,disabled=['Indicator'])
    if st.button('Save economic indicators'):econ.to_csv(ECON,index=False);st.success('Saved')
    st.subheader('Money Market 7-Day Yields');money=st.data_editor(pd.read_csv(MM),use_container_width=True,hide_index=True,disabled=['Fund','Ticker'])
...
st.subheader('Risk-On / Risk-Off Dashboard');cols=st.columns(6)
for col,z in zip(cols,s.get('risk',[])):
    with col:st.metric(z['Asset'],f"{z['Current']:.2f}%" if z['Yield'] else f"{z['Current']:,.2f}",f"{z['Change']:+.2f} pts" if z['Yield'] else f"{z['Change']:+.2%}")
...
st.divider();st.subheader('Economic Indicators');st.dataframe(pd.read_csv(ECON),use_container_width=True,hide_index=True)
st.divider();st.subheader('Cash Alternatives');st.dataframe(pd.read_csv(MM),use_container_width=True,hide_index=True)
